#ifndef DATABASE_H
#define DATABASE_H

#include <Arduino.h>

struct Spice {
  String name;
  int r_val, g_val, b_val;
};

struct RecipeItem {
  int spiceIndex; 
  float quantityGrams; 
};

struct Recipe {
  String name;
  int ingredientCount;
  RecipeItem ingredients[10]; 
};

// --- DATA ---
const int NUM_SPICES = 20;

// Note: Declared 'extern' here, defined in .ino to avoid linker errors if included multiple times
// However, for simplicity in Arduino IDE, we define them static or directly here if header guards used.
static const Spice spices[NUM_SPICES] = {
  {"Paprika",      180, 420, 380}, // 0
  {"Onion Powder", 130, 140, 135}, // 1
  {"Basil",        350, 180, 320}, // 2
  {"Turmeric",     140, 180, 300}, // 3
  {"Oregano",      380, 220, 350}, // 4
  {"Coriander",    250, 280, 300}, // 5
  {"Thyme",        360, 250, 340}, // 6
  {"Ginger",       160, 190, 280}, // 7
  {"Parsley",      340, 170, 330}, // 8
  {"Cardamom",     280, 200, 290}, // 9
  {"Rosemary",     400, 250, 380}, // 10
  {"Clove",        450, 480, 460}, // 11
  {"Garlic",       120, 125, 115}, // 12
  {"Nutmeg",       320, 360, 400}, // 13
  {"Black Pepper", 520, 600, 540}, // 14
  {"Bay Leaves",   330, 230, 310}, // 15
  {"Chili Flakes", 190, 400, 370}, // 16
  {"Star Anise",   420, 490, 470}, // 17
  {"Cumin",        290, 330, 380}, // 18
  {"Cinnamon",     260, 380, 420}  // 19
};

static const Recipe recipes[12] = {
  { "Italian Herbs", 8, { {2, 1.6}, {4, 1.6}, {6, 1.2}, {8, 1.2}, {10, 1.0}, {12, 0.8}, {14, 0.6}, {16, 0.2} }},        
  { "Taco Seas.", 8, { {0, 2.0}, {18, 2.2}, {1, 1.4}, {12, 1.2}, {14, 0.8}, {16, 0.8}, {6, 0.6}, {4, 0.6} }},
  { "Curry Madras", 9, { {3, 2.0}, {5, 1.6}, {18, 1.2}, {0, 0.8}, {7, 0.8}, {12, 0.6}, {14, 0.6}, {16, 0.4}, {9, 0.4} }},
  { "BBQ", 9, { {0, 2.4}, {12, 1.2}, {1, 1.2}, {14, 0.8}, {18, 0.6}, {16, 0.6}, {6, 0.6}, {4, 0.2}, {19, 0.4} }},
  { "Garam Masala", 9, { {9, 2.0}, {18, 1.6}, {5, 1.2}, {19, 0.8}, {14, 0.6}, {11, 0.6}, {13, 0.6}, {15, 0.4}, {17, 0.4} }},
  { "Ras el Hanout", 10, { {18, 1.6}, {5, 1.6}, {19, 0.8}, {0, 0.8}, {7, 0.8}, {9, 0.6}, {11, 0.4}, {13, 0.6}, {3, 0.6}, {14, 0.6} }},
  { "Gyros", 8, { {4, 1.6}, {6, 1.2}, {12, 1.2}, {1, 1.2}, {0, 1.2}, {18, 0.6}, {14, 0.6}, {8, 0.4} }},
  { "Shawarma", 9, { {18, 1.6}, {5, 1.2}, {0, 1.2}, {12, 1.2}, {1, 0.8}, {14, 0.6}, {3, 0.6}, {19, 0.4}, {9, 0.4} }},
  { "ChiliConCarne", 7, { {0, 2.4}, {16, 1.6}, {18, 1.6}, {12, 0.8}, {1, 0.8}, {4, 0.4}, {14, 0.4} }},
  { "Moroc. Tagine", 8, { {18, 1.6}, {5, 1.6}, {0, 1.2}, {3, 1.2}, {19, 0.8}, {7, 0.8}, {14, 0.4}, {9, 0.4} }},
  { "Ind. Tandoori", 8, { {0, 2.0}, {18, 1.4}, {5, 1.2}, {12, 1.2}, {7, 0.8}, {3, 0.6}, {16, 0.6}, {9, 0.2} }},
  { "H. de Provence", 8, { {6, 1.6}, {10, 1.6}, {4, 1.2}, {2, 1.2}, {8, 0.8}, {15, 0.6}, {14, 0.4}, {12, 0.6} }}
};

#endif